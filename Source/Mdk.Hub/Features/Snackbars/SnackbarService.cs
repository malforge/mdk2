...
public class SnackbarService : ISnackbarService
{
    ...
    async Task AnimatePositionAsync(SnackbarWindow snackbar, int targetX, int targetY)
    {
        ...
        for (var i = 1; i <= steps; i++)
        {
            ...
            await Dispatcher.UIThread.InvokeAsync(() => { snackbar.Position = new PixelPoint(targetX, newY); });

            await Task.Delay(delay);
        }

        // Ensure final position is exact
        await Dispatcher.UIThread.InvokeAsync(() => { snackbar.Position = new PixelPoint(targetX, targetY); });
    }
}
