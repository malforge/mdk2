...
public class ProjectConfiguration
{
    ...
    /// </summary>
    public List<string> ConfigurationWarnings { get; init; } = new();
    ...
    public string? GetResolvedOutputPath()
    {
        ...
        {
            Debug.WriteLine("GetResolvedOutputPath: ProjectPath is null or empty");
            return null;
        }
        ...
        if (!string.IsNullOrWhiteSpace(output) && !string.Equals(output, "auto", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                Debug.WriteLine($"GetResolvedOutputPath: Could not extract project name from path: {ProjectPath}");
                return output; // Return the custom path as-is without project name
            }
            return Path.Combine(output, projectName);
        }

        // Resolve "auto" based on project type
        var appData = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
        var projectName2 = Path.GetFileNameWithoutExtension(ProjectPath.Value);

        if (string.IsNullOrEmpty(projectName2))
        {
            Debug.WriteLine($"GetResolvedOutputPath: Could not extract project name from path: {ProjectPath.Value}");
            return null;
        }
        ...
        return type switch
        {
            "programmableblock" => Path.Combine(appData, "SpaceEngineers", "IngameScripts", "local", projectName2),
            "mod" => Path.Combine(appData, "SpaceEngineers", "Mods", projectName2),
            _ => null
        };
    }
}