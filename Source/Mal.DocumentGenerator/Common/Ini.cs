...
public class Ini
{
    ...
    public static bool TryParse(string ini, out Ini result)
    {
        ...
        bool tryReadLine(string text, ref int idx, out string line)
        {
            var start = idx;
            ...
            if (idx < text.Length && text[idx] == '\n')
                idx++;
            line = text.Substring(start, idx - start);
            if (line.EndsWith("\r\n"))
        ...
        while (tryReadLine(ini, ref index, out var line))
        {
            ...
            if (trimmed.StartsWith("[") && trimmed.EndsWith("]"))
            {
                ...
                currentSection = new Section(trimmed.Substring(1, trimmed.Length - 2).Trim());
                if (comment.Length <= 0)
                    continue;
