...
namespace Mdk2.References.Utility
{
    ...
    class Steam
    {
        ...
        /// <param name="verificationFilePath">
        ///     The relative path to a file which existence verify the correctness of the install
        ///     folder.
        /// </param>
        /// <returns></returns>
        public string GetInstallFolder(string subfolderName, string verificationFilePath)
        {
            ...
            try
            {
                ...
                basePath = RegistryReader.GetSteamPath();
            }
            catch (Exception)
            {
                return null;
        ...
        string FindSteam()
        {
            var path = RegistryReader.GetSteamExe();
        ...
        Dictionary<string, object> ReadVdf(string configPath)
        {
        ...
        Dictionary<string, object> ReadVdfObject(string[] lines, ref int pos, bool nested)
        {
            if (nested)
            {
        ...
        string Unquote(string value) => value.Substring(1, value.Length - 2).Replace("\\\"", "\"");

        static KeyValuePair<string, string>? ReadVdfLine(string[] lines, ref int pos)
        {
            if (pos >= lines.Length)
                return null;
